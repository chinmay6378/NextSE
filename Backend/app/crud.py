...
async def get_visible_client(db: AsyncSession, profile: Profile, client_id: uuid.UUID) -> Client:
    """Fetch a client, 404ing if it doesn't exist or is a draft (for non-admins).
    Engineers can access any published client for studying; TestRequest is only
    required for test-taking endpoints."""
