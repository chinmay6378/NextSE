...
export function ClientLearning({ clientId, clientName }: ClientLearningProps) {
  ...
  const [activeVideo, setActiveVideo] = useState<import('@/lib/api/types').YoutubeVideo | null>(null)
  ...
  /* Study material not generated yet (404) or other error */
  if (materialIsError) {
    return (
      <motion.div
      ...
      >
        ...
        <p className="text-muted-foreground text-sm">
          Ask your admin to generate study material for {clientName}.
        </p>
      </motion.div>
    )
  }

  /* Study material is still generating */
  if (studyMaterial?.status === 'generating') {
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.97 }}
        animate={{ opacity: 1, scale: 1 }}
      ...
      >
        ...
        <p className="text-muted-foreground text-sm">
          This takes about a minute. The page will refresh automatically.
        </p>
      </motion.div>
    )
  }
  ...
  /* Generation failed */
  if (studyMaterial?.status === 'failed') {
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.97 }}
        animate={{ opacity: 1, scale: 1 }}
        className="text-center py-16 bg-card border border-red-100 rounded-2xl"
      >
        ...
        <p className="text-muted-foreground text-sm">
          Contact your admin to regenerate the study material for {clientName}.
        </p>
      </motion.div>
    )
  }

  /* No content yet (ready but empty) */
  if (!content) {
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.97 }}
      ...
      >
        ...
        <p className="text-muted-foreground text-sm">
          Check back once an admin has generated it for {clientName}.
        </p>
      </motion.div>
    )
  }
  ...
  return (
    <motion.div className="space-y-7" variants={container} initial="hidden" animate="show">
      ...
      {/* Progress */}
      <motion.div variants={item} className="bg-card border border-border rounded-2xl p-5 shadow-sm">
              ...
              <div className="p-6">
                {selected.kind === 'flashcard' ? (
                  ...
                  <div className="text-sm text-foreground whitespace-pre-wrap leading-relaxed">
                    {selected.kind === 'module' ? selected.content : selected.content}
                  </div>
                )}
